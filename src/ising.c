...
int SpinToFlipKawasakiRandom(IntVector spins, int *flip, int *flip2,int itmax)
{
	...
	*flip=urand(0,Nspins); //One spin
	do
	{
		if (count > maxkaw)
		{
			*flip=urand(0,Nspins); //Chose another spin in case we doesn't find spins with opposite values
			count=0;
		}
		*flip2=urand(0,Nspins-1); //And another spin
		if (*flip2 >= *flip)
			++*flip2;
		++count; //Count in case all the spins we chose have the same value
		++extracounts;
	}
